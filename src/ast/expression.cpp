...
Ternary::Ternary(std::unique_ptr<Expr> left, const Token &left_operator,
...
Logical::Logical(std::unique_ptr<Expr> left, const Token &op,
                 std::unique_ptr<Expr> right)
    : m_left{std::move(left)}, m_operator{op}, m_right{std::move(right)} {}

Call::Call(std::unique_ptr<Expr> callee, const Token &paren,
...
Get::Get(std::unique_ptr<Expr> obj, const Token &name)
    : m_obj{std::move(obj)}, m_name{name} {}

Set::Set(std::unique_ptr<Expr> obj, const Token &name,
...
Object Super::accept(ExprVisitor &visitor) {
